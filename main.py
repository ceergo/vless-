...
NODE_OVERALL_TIMEOUT: float = 90.0  # жёсткий тайм-аут на один узел
...
PROTOCOL_RE = re.compile(r"(?i)\b(vless|vmess|trojan|ss|hy2|tuic)://[^\s]+")
BASE64_BLOCK_RE = re.compile(r"([A-Za-z0-9+/]{50,}={0,2})")


def _decode_base64_to_text(b64_str: str) -> Optional[str]:
    """Пробуем декодировать Base64-блок в текст UTF-8."""
...
def industrial_extractor(source_text: str) -> List[str]:
    ...
    def _extract(text: str, depth: int = 0) -> None:
        # Ограничиваем глубину рекурсии на всякий случай
        if depth > 5:
            return
        ...
        for match in BASE64_BLOCK_RE.finditer(text):
            ...
            if decoded:
                _extract(decoded, depth + 1)

    _extract(source_text)
    return results
...
def extract_server_identity(parsed: ParsedNode) -> Optional[str]:
    """Формирует identity для дедупликации (host:port:protocol:network)."""
    if not parsed.host or not parsed.port:
        return None
    network = parsed.network or "tcp"
    return f"{parsed.host}:{parsed.port}:{parsed.protocol}:{network}"
...
def build_normalized_subscription(raw_text: str) -> Tuple[Dict[str, str], Dict[str, ParsedNode]]:
    ...
    logging.info("Извлечено %d конфигов из RAW-подписок", len(extracted))
...
def classify_result(result: TestResult) -> Optional[str]:
    ...
    """
    if not result.reachable or not result.download_mbps:
        return None
